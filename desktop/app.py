...
def conversion_source(info, target):
    ...
    source_h = min(higher)
    return f"bestvideo[height={source_h}]+bestaudio/best[height={source_h}]"
...
class VidLoomApp(tk.Tk):
    ...
    def _build(self):
        ...
        style.configure("Blue.Horizontal.TProgressbar", troughcolor="#101a3a", background="#3b82f6", bordercolor="#101a3a", lightcolor="#3b82f6", darkcolor="#3b82f6")
        top = tk.Frame(self, bg="#050817", height=72); top.pack(fill="x")
        tk.Label(top, text="▷", fg="#36d1ff", bg="#050817", font=("Segoe UI", 30, "bold")).pack(side="left", padx=(26,8))
        brand = tk.Frame(top, bg="#050817"); brand.pack(side="left")
        tk.Label(brand, text="VidLoom", fg="white", bg="#050817", font=("Segoe UI",22,"bold")).pack(anchor="w")
        tk.Label(brand, text="VIDEO DOWNLOADER", fg="#8d9ac4", bg="#050817", font=("Segoe UI",7,"bold")).pack(anchor="w")
        tk.Label(top, text="Professional local downloader", fg="#8d9ac4", bg="#050817", font=("Segoe UI",10)).pack(side="right", padx=28)
        self._search_bar(); self._platforms()
        body = tk.Frame(self, bg="#050817"); body.pack(fill="both", expand=True, padx=28, pady=(4,20)); body.columnconfigure(0,weight=3); body.columnconfigure(1,weight=2); body.rowconfigure(0,weight=1)
        self.preview = tk.Frame(body,bg="#09122b",highlightthickness=1,highlightbackground="#1d4ed8"); self.preview.grid(row=0,column=0,sticky="nsew",padx=(0,10)); self._preview_contents()
        self.panel = tk.Frame(body,bg="#09122b",highlightthickness=1,highlightbackground="#253b74"); self.panel.grid(row=0,column=1,sticky="nsew",padx=(10,0)); self._download_panel()

    def _search_bar(self):
        wrap=tk.Frame(self,bg="#050817"); wrap.pack(fill="x",padx=36,pady=(8,18))
        self.url=tk.Entry(wrap,bg="#0b1430",fg="#dce6ff",insertbackground="white",relief="flat",font=("Segoe UI",12)); self.url.pack(side="left",fill="x",expand=True,ipady=13,padx=(2,10)); self.url.insert(0,"https://paste-a-public-video-url-here"); self.url.bind("<FocusIn>",self._clear_placeholder)
        tk.Button(wrap,text="GO  →",command=self.fetch,bg="#2563eb",fg="white",activebackground="#3b82f6",relief="flat",font=("Segoe UI",11,"bold"),padx=24,pady=11).pack(side="right")

    def _platforms(self):
        row=tk.Frame(self,bg="#050817"); row.pack(fill="x",padx=40,pady=(0,18))
        platforms=[("▶","YouTube"),("♪","TikTok"),("◎","Instagram"),("f","Facebook"),("𝕏","X"),("●","Reddit"),("V","Vimeo"),("P","Pinterest"),("▣","Twitch")]
        for icon,name in platforms:
            box=tk.Frame(row,bg="#071027",highlightthickness=1,highlightbackground="#182d5d",width=100,height=68); box.pack(side="left",expand=True,fill="x",padx=4); box.pack_propagate(False)
            tk.Label(box,text=icon,fg="#48a8ff",bg="#071027",font=("Segoe UI",19,"bold")).pack(pady=(6,0)); tk.Label(box,text=name,fg="#c6d1ed",bg="#071027",font=("Segoe UI",8)).pack()

    def _preview_contents(self):
        tk.Label(self.preview,text="MEDIA PREVIEW",fg="#7183b0",bg="#09122b",font=("Segoe UI",9,"bold")).pack(anchor="w",padx=20,pady=(18,10))
        self.thumb=tk.Label(self.preview,text="Paste a public URL above\nand click GO",fg="#8291b5",bg="#050817",font=("Segoe UI",18,"bold"),justify="center"); self.thumb.pack(fill="both",expand=True,padx=20,pady=10)
        self.title_label=tk.Label(self.preview,text="",fg="white",bg="#09122b",font=("Segoe UI",14,"bold"),anchor="w",justify="left",wraplength=650); self.title_label.pack(fill="x",padx=20,pady=(4,2))
        tk.Label(self.preview,text="Public media only • DRM/login-protected media is not bypassed",fg="#7f8fb4",bg="#09122b",font=("Segoe UI",9),anchor="w").pack(fill="x",padx=20,pady=(0,18))

    def _download_panel(self):
        tk.Label(self.panel,text="Download Video",fg="white",bg="#09122b",font=("Segoe UI",16,"bold")).pack(anchor="w",padx=20,pady=(18,16))
        tabs=tk.Frame(self.panel,bg="#09122b"); tabs.pack(fill="x",padx=20)
        self.video_btn=tk.Button(tabs,text="▣ Video",command=lambda:self.set_mode("video"),relief="flat",bg="#4f20ff",fg="white",font=("Segoe UI",10,"bold"),pady=10); self.video_btn.pack(side="left",fill="x",expand=True,padx=(0,5))
        self.audio_btn=tk.Button(tabs,text="♫ Audio",command=lambda:self.set_mode("audio"),relief="flat",bg="#101b3a",fg="#a8b4d4",font=("Segoe UI",10,"bold"),pady=10); self.audio_btn.pack(side="left",fill="x",expand=True,padx=(5,0))
        tk.Label(self.panel,text="Select Quality",fg="white",bg="#09122b",font=("Segoe UI",10,"bold")).pack(anchor="w",padx=20,pady=(22,6))
        self.quality_list=tk.Listbox(self.panel,bg="#071027",fg="#dce6ff",selectbackground="#1d4ed8",selectforeground="white",relief="flat",height=9,font=("Segoe UI",10)); self.quality_list.pack(fill="x",padx=20); self.quality_list.bind("<<ListboxSelect>>",self._quality_selected)
        self.quality_hint=tk.Label(self.panel,text="Analyze a URL to see REAL available qualities.",fg="#8291b5",bg="#09122b",font=("Segoe UI",9),wraplength=330,justify="left"); self.quality_hint.pack(anchor="w",padx=20,pady=(8,12))
        self.download_btn=tk.Button(self.panel,text="↓  DOWNLOAD",command=self.download,state="disabled",relief="flat",bg="#2563eb",fg="white",font=("Segoe UI",12,"bold"),pady=12); self.download_btn.pack(fill="x",padx=20,pady=(4,12))
        self.progress=ttk.Progressbar(self.panel,style="Blue.Horizontal.TProgressbar",maximum=100); self.progress.pack(fill="x",padx=20,pady=(4,8))
        self.status=tk.Label(self.panel,text="Ready",fg="#aab8d7",bg="#09122b",font=("Segoe UI",9),wraplength=330,justify="left"); self.status.pack(anchor="w",padx=20,pady=(0,8))
        self.folder_label=tk.Label(self.panel,text=f"Save: {self.output_dir}",fg="#64759d",bg="#09122b",font=("Segoe UI",8),wraplength=330,justify="left"); self.folder_label.pack(anchor="w",padx=20)
        tk.Button(self.panel,text="Choose folder",command=self.browse,bg="#101b3a",fg="#b8c5e4",relief="flat",pady=7).pack(fill="x",padx=20,pady=(8,18))
    ...
    def _show_info(self,info,heights):
        ...
        self.quality_value=None; self.title_label.config(text=info.get("title") or "Untitled media"); platform=info.get("extractor_key") or info.get("extractor") or "Source"; duration=info.get("duration_string") or "Unknown duration"; self.meta_label.config(text=f"{platform}  •  {duration}  •  Public media only") if hasattr(self,'meta_label') else None; self.quality_hint.config(text="Only real source heights are selectable. No silent downgrade."); self.status.config(text="Media detected. Select an available quality."); self._set_thumbnail(info.get("thumbnail"))
    ...
    def progress_hook(self,data):
        ...
        elif data.get("status")=="finished": self.after(0,lambda:self.progress.config(value=100))
